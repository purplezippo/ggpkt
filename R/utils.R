  ...
  ggplot_object_name = "mywaterfall")
{
  if (!is.null(.data)) {
    ...
    if (!xor(num_cols[1], num_cols[2]) || sum(char_cols[1:2],
      ...
      stop("`.data` did not contain exactly one numeric column and exactly one character or factor ",
           ...
           const_width_name(dat)[1], "'\t",
           sapply(dat, class)[1], "\n\t", "2nd column: '",
           const_width_name(dat)[2],
           "'\t", sapply(dat, class)[2])
    }
    if (num_cols[1L]) {
  ...
  if (!(length(values) == length(labels) && length(values) == length(rect_text_labels))) {
  ...
  if (fill_by_sign) {
    ...
    fill_colours <- ifelse(values >= 0, gg_color_hue(2)[2], gg_color_hue(2)[1])
  }
  else {
    if (is.null(fill_colours)) {
  ...
  if (!calc_total) {
    p <- if (scale_y_to_waterfall) {
      ggplot2::ggplot(
        data.frame(x = c(labels, labels), y = c(south_edge, north_edge)),
        ggplot2::aes_string(x = "x",y = "y")
      )
    }
    else {
      ggplot2::ggplot(data.frame(x = labels, y = values), ggplot2::aes_string(x = "x", y = "y"))
    }
    ...
    p <- if (scale_y_to_waterfall) {
      ggplot2::ggplot(
        data.frame(
          x = c(labels, total_axis_text, labels, total_axis_text),
    ...
    if (abs(values[i]) > put_rect_text_outside_when_value_below) {
      p <- p + ggplot2::annotate("text", x = i, y = 0.5 *
                                 ...
                                 label = ifelse(rect_text_labels[i] == values[i],
                                                ifelse(values[i] < 0,
                                                       paste0("<U+2212>", -1 * values[i]),
                                                       values[i]),
                                                rect_text_labels[i]),
                                 size = rect_text_size/(5/14),
                                 colour = rect_text_color)
    }
    else {
  ...
  if (calc_total) {
      ...
      ggplot2::annotate("text", x = number_of_rectangles +
                        ...
                        label = ifelse(total_rect_text == sum(values),
                                       ifelse(north_edge[number_of_rectangles] < 0,
                                              paste0("<U+2212>", -1 * north_edge[number_of_rectangles]),
                                              north_edge[number_of_rectangles]),
                                       total_rect_text),
    ...
    if (draw_lines) {
      p <- p + ggplot2::annotate(
        "segment", x = number_of_rectangles - anchor_left,
